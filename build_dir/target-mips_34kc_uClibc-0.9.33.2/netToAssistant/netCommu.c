...
void* ThreadUDP(void*  pVoid)
{
    ...
    while (m_bExitUDP)
    {
        ...
        if(strcmp(recvmsg,READID)==0)
        {
            ...
            sprintf(cBuf, "INVT-<%s,%s,%d>", mqttInfo->strClientID, mqttInfo->strVer,PORT_UDP);
            strcpy(sendmsg, cBuf);
#ifdef DEBUG_INFO
                    printf("UDP sendBuf = %s, len = %d , port=%d\n", sendmsg, strlen(sendmsg),PORT_UDP);
#endif
        ...
        while((len=recv(client_sockfd,buf,BUFSIZ,0))>0)
        {
            ...
            if (len < 50) //传指令
            {
                if(0 == strcmp(buf,READID)) //如果是读取设备ID命令
                {
                    ...
                    ReadMqttConfigOnly(MQTT_CONFIG);
                    char cBuf[MAXDATASIZE];
                    sprintf(cBuf, "INVT-<%s,%s,%d>",mqttInfo->strClientID, mqttInfo->strVer, PORT_TCP);
                    strcpy(sendBuf, cBuf);

#ifdef DEBUG_INFO
                    printf("TCP sendBuf = %s, len = %d ,port = %d\n", sendBuf, strlen(sendBuf), PORT_TCP);
#endif
