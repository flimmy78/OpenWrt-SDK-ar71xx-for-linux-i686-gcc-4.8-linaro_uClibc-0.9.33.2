...
{
#ifdef DEBUG_INFO
        printf("TCP accept TCP client port = %d \n",PORT_APP_TCP);
#endif
    ...
    char sendBuf[BUFSIZ];
    unsigned char sendTCPBuf[BUFSIZ];
...
#ifdef DEBUG_INFO
        printf("TCP accept client %s,port=%d\n",inet_ntoa(remote_addr.sin_addr),PORT_APP_TCP);
#endif
...
//                int iRet = strToHexTCP(buf, cmdData);
                ...
                ivalue = strToHexTCP(sendBuf, sendTCPBuf);
                printf("sendTCPBuf  = ");
                for(i=0;  i < ivalue; i++)
                {
                    printf("%02x ",sendTCPBuf[i]);
                }
#ifdef DEBUG_INFO
                //printf("\nsendBuf = %s TCP rev DATA 9090909090\n", sendBuf);
#endif
                //if(send(client_sockfdtcp,sendBuf,strlen(sendBuf),0)<0)
                 if(send(client_sockfdtcp,sendTCPBuf,ivalue,0)<0)
                {
#ifdef DEBUG_INFO
...
{
    printf("getUpBufTCP:: id = %02x, iAddr = %d, iLen = %d \n", id, iAddr, iLen);
    printf("drive_data[EN_COM1].iDeviceNum=%d\n", drive_data[EN_COM1].iDeviceNum);   //lf_debug
    char sendBuf[BUFSIZ]={0};
    UCHAR m_ucBuf[MAX_SENDNORMAL_BUFFER_SIZE];
    int i = 0;
    for (i = 0; i < drive_data[EN_COM1].iDeviceNum; i++)
    {
      //  printf("drive_data[EN_COM1].deviceInfo[%d].iDeviceID=%d\n",  i, drive_data[EN_COM1].deviceInfo[i].iDeviceID);//lf_debug
        id = drive_data[EN_COM1].deviceInfo[i].iDeviceID;
        if (id == drive_data[EN_COM1].deviceInfo[i].iDeviceID)
        {
            ...
            {
  //       printf("drive_data[EN_COM1].deviceInfo[%d].iDeviceNum=%d\n",  i, drive_data[EN_COM1].deviceInfo[i].iDeviceNum);//lf_debug
                if (bFlag)
                {
                    if (k < iLen - 1)
                    {
