...
bool bFlagUpInit = false;                               //首次上报标志  上报之后置 true
int iDevFailedNum = 1;
bool bModbusflag;
...
int main()
{
    ...
    Connect(); //读取串口配置,打开串口
    QCommuSerial_init(SFunCallback_RecvNormal);   /***通讯口初始化并 绑定接收数据回调函数，lf_debug
                                                                                                接收到数据之后回调函数 SFunCallback_RecvNorma 执行 **/
    //再创建通讯任务管理线程
    //关闭串口时先要再关闭串口 ,再退出管理线程 Disconnect()  ExitThread_Manage()
    CleanCurrentTask();//清空当前任务
    CreateThread_Manage();                                       /*** 创建任务管理线程  检测task 状态,  lf_debug
                                                                                        执行  Serial_DealWith_SendNormal();下发指令给从站 */
    initThreadHttp();
    initThreadTCP();
    ...
    int iOldSec2= time( &nowtime );//上传全部数据计时 (上电五分钟后)
    int iOldSec3= time( &nowtime );// 定时上传全部数据计时
    ...
    while(1)
    {
        //处理远程下发命令
#ifdef DRIVE_COM1
        getDownCmdAndAction(EN_COM1_VALUE, EN_COM1, DRIVE1_CMD);      //( 1,0,ctronl )
#else
        getDownCmdAndAction(EN_COM2_VALUE, EN_COM2, DRIVE2_CMD);
#endif

        int iCurrentSec = time( &nowtime );                                               // iOldSec1 每隔一段时间扫描所有终端设备，数目和时间间隔由配置表来定
      ...
      //  printf("drive_data[EN_COM1].iDeviceNum= %d\n",drive_data[EN_COM1].iDeviceNum);
        ...
        if (!bFlagUpInit)
        {
            if (iCurrentSec- iOldSec2 > INIT_TIME * 60)                               // iOldSec2 上电3分钟后上传全部数据
            {
                printf("lf_debug The first time Upload all data****iCurrentSec - iOldSec2 =%d\n",iCurrentSec - iOldSec2 );
                bFlagUpInit = true;
                iOldSec5= time( &nowtime );//定时存储历史数据                   //上传全部数据之后 iOldSec5 开始计时定时 存储数据
#ifdef DRIVE_COM1
#ifdef DEBUG_MAIN
...
#endif
                    setDownDriver1(2);                 //  首次上报全部数据之后，置pStation->downData.iFlagCom1 = 2
#ifdef DEBUG_MAIN
...
#endif
        if (iCurrentSec- iOldSec3 >= SerialInfo->iUpAllDataTime)                                            //   iOldSec3 定时上传全部数据
        {
     ...
     //   upDateChangeData(EN_COM1);
        ...
        if (bFlagUpInit)
        {
            ...
            else
            {
                if(getDownDriver1() == 0)
                {
                    if(iCurrentSec - iOldSec5 >= SerialInfo->iHisDateSave*60)                          //iOldSec5 定时将所有数据存入共享内存
                    {
...
//从共享内存中获取下发指令，并解析执行
void getDownCmdAndAction(EN_DRIVE_NUM_VALUE EN_VALUE, EN_DRIVE_NUM EN_NUM, char* cCmdValue)    //( 1,0,ctronl )
{
#ifdef DEBUG_MAIN
    ...
    if (EN_COM1 == EN_NUM)
    {
        ...
        if (1== getDownDriver3())                //lf_debug共享内存中有下发指令时执行
        {
#ifdef DEBUG_MAIN
            ...
            memcpy(downCmd, getDownCmdCom1(),strlen(getDownCmdCom1()));
           // clrDownCmdCom2();         //此处可能有问题应该为   clrDownCmdCom1();   lf_debug
             clrDownCmdCom1();
//#ifdef DEBUG_MAIN
            printf("downCmd = %s, iDownLen = %d \n", downCmd, getDownCmdLengthCom1());
//#endif
